...
case class nodeDestroyed(initiator: ActorRef,predecessor: ActorRef)
...
case class setFingerTable(map: SortedMap[Int, ActorRef], map1: SortedMap[Int, ActorRef])
...
class Nodes(Id: String, numNodes: Int) extends Actor {
  var destroyedNode:ActorRef=null
  val num = numNodes
  val ID = Id
  val hsahedKey: String = ""
  var fingerTable = SortedMap.empty[Int, ActorRef]
  var successorTable = SortedMap.empty[Int, ActorRef]
...
*/  case setSuccessor(newSuccessor)=>{
      succ=newSuccessor
        }
    ...
    case findFileWithHop(initiator, k, cnt) => {
      ...
      if (!flag) {
        ...
        if (minIndex != 0) {
          flag = true
          count+=1
          if(fingerTable(minIndex) !=destroyedNode)
            fingerTable(minIndex) ! findFileWithHop(initiator, k,count)
          else
            successorTable(minIndex) ! findFileWithHop(initiator, k,count)
        }
      }
      ...
      if (!flag) {
        ...
        count += 1
        if(succ !=destroyedNode)
          succ ! findFileWithHop(initiator, k,count)
        else
          successorTable(1) ! findFileWithHop(initiator, k,count)
    ...
    case nodeDestroyed(init,pred)=>{
      destroyedNode=init
      if(self!=pred){
        succ! nodeDestroyed(init,pred)
      }
    }
    ...
    case setFingerTable(fingTable, sucTable) => {
      fingerTable = fingTable
      successorTable = sucTable
...
class Chord(totalnodes: Int, NumRequests: Int) extends Actor {
  ...
  def populateFingertable(nodes: ArrayBuffer[ActorRef], numNodes: Int) = {
    ...
    for (i <- 0 to numNodes - 1) {
      ...
      var successorTable = SortedMap[Int, ActorRef]()
      ...
      while (l <= m) {
        ...
        var entry1 = entry + 1;
        if(indexMap.contains(entry1)){
          r = indexMap(entry1)
        }else{
          entry1 = (entry1 + 1) % (math.pow(2, m).toInt)
          while(indexMap.contains(entry1) != true){
            entry1 = (entry1 + 1) % (math.pow(2, m).toInt)
          }

          r = indexMap(entry1)
        }
        successorTable += (l -> sortedNodes(r))
      ...
      val successorTableSortedMap = SortedMap.empty[Int, ActorRef] ++ successorTable

      // fingerTableMap ++=  fingerTableSortedMap
      //println("fingertable for node " + sortedNodes(i).path.name + " -> " + fingerTableSortedMap)
      //println("successorTableSortedMap for node " + sortedNodes(i).path.name + " -> " + successorTableSortedMap)
      sortedNodes(i) ! setFingerTable(fingerTableSortedMap, successorTableSortedMap)
  ...
  def receive = {
    ...
    case "initialize" => {
      ...
      var destroyedNode = Random.nextInt(sortedNodes.length)
      context.stop(sortedNodes(destroyedNode))
      println("Node id " + Integer.parseInt(sortedNodes(destroyedNode).path.name) + " has failed")
